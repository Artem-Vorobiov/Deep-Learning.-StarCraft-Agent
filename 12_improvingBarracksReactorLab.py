...
#   
#   Как выделить Барак с Реактором и отдельно барак с Лабораторией
#
#
    async def improve_barracks(self):   
        print('\n morph',  self.barrack_morph)
        ...
        for BR in self.units(BARRACKS).ready:
            if not self.tags:
                ...
                self.barrack_morph['BARRACKSTECHLAB'] = BR.tag
                # print('\nTAGS',  self.barrack_morph)
            elif not self.units(BARRACKSREACTOR).exists:
                await self.do(BR.build(BARRACKSREACTOR))
                print('\n\t\t\t\t BARRACKSREACTOR')
                self.tags.add(BR.tag)
                self.barrack_morph['BARRACKSREACTOR'] = BR.tag
                # print('\nTAGS',  self.barrack_morph)
