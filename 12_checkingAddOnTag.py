...
class NN(sc2.BotAI):
    async def on_step(self, iteration):
        ...
        await self.check_add_on_tag()

    async def check_add_on_tag(self):
        if self.units(BARRACKS).ready:
            print(('\n\t\t\t\ BASE:', self.units(BARRACKS).add_on_tag))
            for barr in self.units(BARRACKS):
                print('\n\t\t\t\ Checking add on tag:', barr.add_on_tag)
                print('\n\t\t\t\ Checking add on tag:', type(barr.add_on_tag))
                #   When first battacks ends  - 4367319041
                #   When second barracks ends - 4363649025
